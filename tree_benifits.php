...
<head>
    ...
    <title>மரங்கள் மற்றும் அதன் பயன்கள் - Tree's and its Benifits</title>
        ...
        <div id="content">
            ...
            <?php 
               $con=new mysqli('localhost','id18233341_root','t1$ROHySi^^<a7|G','id18233341_vnr');
                ...
                $sql="SELECT * FROM tree_details ORDER BY treename";
                $result=mysqli_query($con,$sql);
                if($result){
                        while($row=mysqli_fetch_assoc($result))
                        {
                            $treename=$row['treename'];
                            $benifits=$row['benifits'];
                            $photo=$row['photo'];
                                echo "<div class='col-md-4 col-lg-4 col-sm-4 mb-3'>
                                <a href='planted_trees.php' class='card shadow-lg'>
                                    <div class='card-body'>
                                    <div class='p'><b>$treename :</b></div><br>
                                        <center><img src='tree_image/$photo' class='img-fluid float-start'></center><br>
                                        <div class='p'><b>பயன்கள்</b><br>$benifits</div>
                                    </div>
                                </a>
                                </div>";
                        }         
